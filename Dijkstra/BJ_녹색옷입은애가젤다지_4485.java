...
public class Main {
	...
	static int BFS() {
		...
		PriorityQueue<Info> Q = new PriorityQueue<>((e1, e2)->e1.sum-e2.sum);
		...
		while(!Q.isEmpty()) {
			...
			for(int i=0; i<4; i++) {
				int xx = x+dx[i];
				int yy = y+dy[i];
				if(xx<0 || xx>=n || yy<0 || yy>=n || visited[xx][yy] || min[x][y]+map[xx][yy]>min[xx][yy]) continue;
				min[xx][yy] = min[x][y]+map[xx][yy];
